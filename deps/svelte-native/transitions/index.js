...
function asSvelteTransition(node, delay = 0, duration = 300, curve = CoreTypes.AnimationCurve.linear, nativeAnimationProps, applyNativeAnimationProps) {
    let svelteAnim = {
        delay: delay,
        duration: duration,
    ...
    if (typeof curve == "string") {
        switch (curve) {
            case CoreTypes.AnimationCurve.ease:
                svelteCurve = ease;
                break;
            case CoreTypes.AnimationCurve.easeIn:
                svelteCurve = ease_in;
                break;
            case CoreTypes.AnimationCurve.easeOut:
                svelteCurve = ease_out;
                break;
            case CoreTypes.AnimationCurve.easeInOut:
                svelteCurve = ease_in_out;
                break;
            case CoreTypes.AnimationCurve.linear:
                svelteCurve = linear;
                break;
            default:
...
function fade(node, { delay = 0, duration = 400 }) {
    ...
    return asSvelteTransition(node, delay, duration, CoreTypes.AnimationCurve.linear, (t) => ({
        opacity: t * o
    }));
}
function fly(node, { delay = 0, duration = 400, easing = CoreTypes.AnimationCurve.easeOut, x = 0, y = 0 }) {
...
export { asSvelteTransition, easing as easings, fade, fly, slide };
